...
function BottomNav({
  ...
  return (
    <div className="border-t border-white/[0.06] bg-[#0d1110]/90 px-2 pb-2 pt-1.5 backdrop-blur-md">
      <div className="flex items-end justify-around">
        {items.map((item, index) => (
          <div
            key={item.label}
            className={`flex flex-col items-center gap-0.5 ${index === active ? "text-white" : "text-white/35"}`}
          >
            {item.icon}
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span
                    className={`mt-0.5 inline-block rounded-md px-1.5 py-0.5 text-[9px] font-semibold ${item.badgeBg}`}
                  >
                    {item.badge}
                  </span>
                </div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex -space-x-1">
                {[0, 1, 2].map((index) => (
                  <div
                    key={index}
                    className="flex h-5 w-5 items-center justify-center rounded-full border border-[#0d1110] bg-white/15 text-[8px] font-bold text-white"
                  >
                    {index + 1}
                  </div>
                ))}
...
export function HeroPhoneMockup({ locale }: { locale: MarketplaceLocale }) {
  return (
    <div className="relative mx-auto w-[280px] max-w-[300px]">
      ...
      >
        ...
        <div className="relative h-full w-full overflow-hidden" style={{ borderRadius: 38 }}>
          <ShortlistScreen locale={locale} />
        </div>
        ...
        <div
          ...
          style={{
            borderRadius: 44,
            background:
              "linear-gradient(135deg, rgba(255,255,255,0.10) 0%, rgba(255,255,255,0.02) 30%, transparent 55%)",
          }}
        />
        <div
  ...
  return (
    ...
    >
      ...
      <div className="border-l border-black/10 pl-3">
        <p className="text-[9px] font-medium leading-none text-black/60">{copy.common.downloadOn}</p>
        <p className="mt-0.5 text-[15px] font-semibold leading-none tracking-tight text-black">
  ...
  return (
    ...
    >
      ...
      <div className="border-l border-white/10 pl-3">
        <p className="text-[9px] font-medium leading-none text-white/50">{copy.common.getItOn}</p>
        <p className="mt-0.5 text-[15px] font-semibold leading-none tracking-tight text-white">
          {copy.common.googlePlay}
        </p>
      </div>
    </a>
  );
