    ...
    <div className="grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
      ...
      <section className="grid gap-4">
        <div className="flex flex-col gap-3 rounded-3xl border border-line bg-white p-5 shadow-card sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-bold text-ink">Ranked {label}</p>
        ...
        {filtered.length === 0 && (
          <div className="rounded-3xl border border-line bg-white p-10 text-center shadow-card">
            <p className="text-sm font-bold text-ink">No offers match your filters</p>
            <p className="mt-2 text-sm text-ink-secondary">Try adjusting your criteria above.</p>
          </div>
        )}
